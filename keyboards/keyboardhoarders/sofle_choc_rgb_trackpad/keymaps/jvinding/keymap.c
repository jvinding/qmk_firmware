...
#include "jvinding.h"
#include "combos.c"
#define KEYMAP_OVERRIDE_POST_INIT
#include "jvinding.c"
...
typedef union {
    uint32_t raw;
    struct {
        bool trackpad_disabled : 1;
    };
} user_config_t;

static bool jv_trackpad_disabled = false;
...
[JV_BASE] = LAYOUT(
  ...
  MS_BTN1, LGUI_T(KC_A),        LALT_T(KC_R),   LCTL_T(KC_S),    LSFT_T(KC_T),    HYPR_T(KC_D),    HYPR_T(KC_H),   RSFT_T(KC_N),   RCTL_T(KC_E),   RALT_T(KC_I),   RGUI_T(KC_O),   MS_BTN2,
...
[JV_QWERTY] = LAYOUT(
  ...
  MS_BTN1, LGUI_T(KC_A),        LALT_T(KC_S),   LCTL_T(KC_D),    LSFT_T(KC_F),    HYPR_T(KC_G),    HYPR_T(KC_H),   RSFT_T(KC_J),   RCTL_T(KC_K),   RALT_T(KC_L),   RGUI_T(KC_QUOT),MS_BTN2,
  MS_WHLD, LT(JV_BUTTON,KC_Z),  KC_X,           KC_C,            KC_V,            KC_B,            KC_TRNS, KC_TRNS,  KC_N,          KC_M,           KC_COMM,        KC_DOT,         LT(JV_BUTTON,KC_SLSH), MS_WHLR,
  KC_NO,   KC_NO,               LT(JV_FUN,KC_DEL),  LT(JV_NUM,KC_BSPC), LT(JV_SYM,KC_ENT),   LT(JV_MOUSE,KC_TAB), LT(JV_NAV,KC_SPC), LT(JV_MEDIA,KC_ESC), KC_NO, TG(JV_GAME)
),

// ── MAC (overlay: swap LCTL↔LGUI back to Mac-style on Colemak-DH positions) ──
[JV_OSALT] = LAYOUT(
  KC_NO,   KC_NO,               KC_NO,          KC_NO,           KC_NO,           KC_NO,           KC_NO,          KC_NO,          KC_NO,          KC_NO,          KC_NO,          KC_NO,
  MS_WHLU, KC_TRNS,             KC_TRNS,        KC_TRNS,         KC_TRNS,         KC_TRNS,         KC_TRNS,        KC_TRNS,        KC_TRNS,        KC_TRNS,        KC_TRNS,        MS_WHLL,
  MS_BTN1, LCTL_T(KC_A),        KC_TRNS,        LGUI_T(KC_S),    KC_TRNS,         KC_TRNS,         KC_TRNS,        KC_TRNS,        RGUI_T(KC_E),   KC_TRNS,        RCTL_T(KC_O),   MS_BTN2,
...
[JV_MOUSE] = LAYOUT(
  ...
  JV_TPTOG,KC_NO,               MS_BTN3,        MS_BTN1,         MS_BTN2,         JV_NA,           JV_NA,          JV_NA,          KC_NO,          TG(JV_GAME)
),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (keycode == JV_TPTOG && record->event.pressed) {
        user_config_t cfg;
        cfg.raw = eeconfig_read_user();
        cfg.trackpad_disabled = !cfg.trackpad_disabled;
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    ...
    [JV_QWERTY]   = { ENCODER_CCW_CW(KC_TRNS, KC_TRNS), ENCODER_CCW_CW(KC_TRNS, KC_TRNS) },
    [JV_OSALT]         = { ENCODER_CCW_CW(KC_TRNS, KC_TRNS), ENCODER_CCW_CW(KC_TRNS, KC_TRNS) },
